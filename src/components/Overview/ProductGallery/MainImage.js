...
class MainImage extends Component {
  ...
  renderCurrentImage = () => {
    if (this.props.thumbnails.length > 0) {
      return (
        <Img
          src={[
            this.props.thumbnails[this.props.selectedThumbnailIndex].url,
  ...
  renderLeftArrow = () => {
    if (this.props.selectedThumbnailIndex !== 0) {
      return <Icon name="arrow left" size="large" value="arrowLeft" />;
    }
  };

  renderRightArrow = () => {
    if (
      this.props.selectedThumbnailIndex !==
      this.props.thumbnails.length - 1
    ) {
      return <Icon name="arrow right" size="large" value="arrowRight" />;
    }
  };
  ...
  render() {
    return (
      <div className="image-wrapper">
        ...
        <div className="image-viewer">{this.renderCurrentImage()}</div>
        <Icon
          name="expand"
          size="large"
          onClick={() => this.props.openExpandedView()}
