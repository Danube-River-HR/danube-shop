...
class ProductGallery extends Component {
  ...
  changeByArrow = (arrowChoice) => {
    if (this.state.thumbnails.length !== 0) {
      if (
        arrowChoice === "arrowLeft" &&
        this.state.selectedThumbnailIndex > 0
      ) {
        this.setState({
          selectedThumbnailIndex: this.state.selectedThumbnailIndex - 1,
        });
      } else if (
        arrowChoice === "arrowRight" &&
        this.state.selectedThumbnailIndex < this.state.thumbnails.length - 1
      ) {
        this.setState({
          selectedThumbnailIndex: this.state.selectedThumbnailIndex + 1,
        });
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      this.state.selectedThumbnailIndex === -1
    ) {
      this.setState({
      ...
      if (
        this.props.selectedStyle.style_id !== prevProps.selectedStyle.style_id
      ) {
        this.setState({
          thumbnails: this.props.selectedStyle.photos,
          selectedThumbnailIndex: 0,
...
const mapStateToProps = ({ overallData, selectedStyle }) => {
  return {
    overallData,
    selectedStyle,
  };
